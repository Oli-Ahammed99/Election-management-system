...
const staffCredentials = {
  apo: { username: "apo", password: "apo123" },
  po: { username: "po", password: "po123" },
  aro: { username: "aro", password: "aro123" },
  ro: { username: "ro", password: "ro123" },
...
function canAccessTab(tab) {
  ...
  return tab.dataset.role === authState.loggedInRole;
}
...
function updateAccessUI() {
  ...
  authStatus.textContent = authState.loggedInRole
    ? `Current access: ${authState.loggedInRole.toUpperCase()}`
    : "Current access: Voter/Public only";
}

tabs.forEach((tab) =>
  tab.addEventListener("click", () => {
    if (!canAccessTab(tab)) {
      authMessage.className = "info error";
      authMessage.textContent = "Access denied. Please login with the required officer/admin role.";
      return;
    }
...
document.getElementById("staff-login-form").addEventListener("submit", (e) => {
  ...
  const expected = staffCredentials[role];

  if (username !== expected.username || password !== expected.password) {
    authMessage.className = "info error";
    authMessage.textContent = "Invalid credentials for selected role.";
    return;
  }

  authState.loggedInRole = role;
  authMessage.className = "info success";
  authMessage.textContent = `Login successful as ${role.toUpperCase()}.`;
  updateAccessUI();

  const allowedTab = [...tabs].find((tab) => tab.dataset.role === role);
  if (allowedTab) {
    switchTab(allowedTab.dataset.target);
  }
});

document.getElementById("logout-btn").addEventListener("click", () => {
  authState.loggedInRole = null;
  authMessage.className = "info";
  authMessage.textContent = "Logged out. Only voter/public module is accessible now.";
  updateAccessUI();
  switchTab("voter-module");
});
